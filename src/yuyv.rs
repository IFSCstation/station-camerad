...
fn yuyv_to_rgb8_scalar(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    let mut s = 0;
    for y in 0..height {
...
mod neon {
    ...
    #[inline(always)]
    unsafe fn yuyv_8px(s: *const u8, d: *mut u8) {
    ...
    #[inline(always)]
    unsafe fn yu12_2px(y_ptr: *const u8, u_val: u8, v_val: u8, d: *mut u8) {
        let y0 = *y_ptr as i32;
        ...
        let base0 = K_Y * (y0 - 16);
        let base1 = K_Y * (y1 - 16);
        *d = clamp8(div_scale(base0 + K_V_R * v));
        *d.add(1) = clamp8(div_scale(base0 - K_V_G * v - K_U_G * u));
        *d.add(2) = clamp8(div_scale(base0 + K_U_B * u));
        *d.add(3) = clamp8(div_scale(base1 + K_V_R * v));
        *d.add(4) = clamp8(div_scale(base1 - K_V_G * v - K_U_G * u));
        *d.add(5) = clamp8(div_scale(base1 + K_U_B * u));
    }

    #[inline(always)]
    unsafe fn yuyv_4px(s: *const u8, d: *mut u8) {
        let b = vld1_u8(s);
        let uzp = vuzp_u8(b, b);
        let y8 = uzp.0;
        let c8 = uzp.1;
    ...
    pub unsafe fn yuyv_to_rgb8_neon(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
        let groups8 = width / 8;
        let tail4 = (width % 8) / 4;
        for y in 0..height {
            ...
            for g in 0..groups8 {
                yuyv_8px(
                    row_in.as_ptr().add(g * 16),
                    row_out.as_mut_ptr().add(g * 24),
                );
            }
            if tail4 > 0 {
                yuyv_4px(
                    row_in.as_ptr().add(groups8 * 16),
                    row_out.as_mut_ptr().add(groups8 * 24),
                );
            }
        }
    }
    ...
    pub unsafe fn yuv420_to_rgb8_neon(
        ...
        for row in 0..height {
            ...
            for g in 0..pixel_pairs {
                ...
                yu12_2px(y_row.add(yi), u_val, v_val, row_out.add(g * 6));
            }
...
pub fn yuv420_to_rgb8(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    ...
    yuv420_to_rgb8_scalar(y_plane, u_plane, v_plane, dst, width, height);
}

pub fn rgb24_to_rgb8(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    assert!(src.len() >= width * height * 3);
    assert!(dst.len() >= width * height * 3);
    dst[..src.len()].copy_from_slice(src);
}

pub fn yuyv_to_rgb8(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    assert_eq!(width % 2, 0);
    ...
    if width >= 4 && width.is_multiple_of(4) && fast_path_available() {
        unsafe {
            neon::yuyv_to_rgb8_neon(src, dst, width, height);
        }
        return;
    }
    yuyv_to_rgb8_scalar(src, dst, width, height);
}
...
mod tests {
    ...
    fn yuv420_red_pixel() {
        ...
        super::yuv420_to_rgb8(&frame, &mut dst, w, h);
        for px in 0..4 {
            let r = dst[px * 3];
            let g = dst[px * 3 + 1];
            let b = dst[px * 3 + 2];
    ...
    fn yuv420_neon_matches_scalar_byte_exact() {
        ...
        for trial in 0..64 {
            for w in [8usize, 16, 32] {
                ...
                super::yuv420_to_rgb8(&src, &mut fast, w, h);
                ...
                for i in 0..fast.len() {
                    if fast[i] != scalar[i] {
                        ...
                        if mismatches <= 5 {
                            ...
                            let ch = i % 3;
                            panic!(
                                "yuv420 mismatch trial={trial} w={w} px={px} ch={ch}: neon={} scalar={}",
                                fast[i], scalar[i]
                            );
                        }
                    }
    ...
    fn neon_matches_scalar_byte_exact() {
        ...
        for trial in 0..64 {
            ...
            for w in [8usize, 16] {
                ...
                let mut scalar = vec![0u8; w * h * 3];
                let src_w = &src[..w * h * 2];
                yuyv_to_rgb8(src_w, &mut fast, w, h);
                yuyv_to_rgb8_scalar(src_w, &mut scalar, w, h);
                for i in 0..fast.len() {
                    if fast[i] != scalar[i] {
                        mismatches += 1;
                        if mismatches <= 5 {
                            ...
                            let ch = i % 3;
                            panic!(
                                "mismatch trial={trial} w={w} px={px} ch={ch}: fast={} scalar={}",
                                fast[i], scalar[i]
                            );
                        }
                    }
